...
In Fig.~\ref{fig:pa_degree}, $p_e(d)\propto d^{1.08}$, $p_e(d_{in})\propto d_{in}^{0.92}$.
...
From Fig.~\ref{fig:pa_pagerank}, we see that $p_e(pr)\propto pr^\tau$, $\tau = 1.00$. This result shows that in our Wikipedia network, new edges attach their destinations preferentially to the destination's PageRank score.


As PageRank turned out to be a good measure of importance for Preferential Attachment, we can easily attempt to modify the PA generation model given by \eqref{1}, substituting $\frac{d_i}{\sum\limits_{k}d_k}$ with $\frac{r_i}{\sum\limits_{k}r_k}=r_i$, which gives us
\begin{equation}
\label{5}
